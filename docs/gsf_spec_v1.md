...
and strand consistency restart per gene block; since protocol A33 gene blocks **may overlap** (antisense, nested and same-strand overlaps are labelled) and the block key `(gene_start, gene_end, canonical_block)` must be non-decreasing — `canonicalize_v3()` sorts by it, the decoding grammar enforces only `start >= previous start` and the output is canonicalised; blocks of one strand with an identical CDS signature are collapsed to one (`dup_collapsed`). Caps: 96 genes per window, 8,192 v3 tokens
...
split comes from 1,032,192-nt genomic blocks (seeded 75/10/15 per species; most restrictive among overlapped blocks; since protocol A31 strict held-out loci do not force blocks to test — they are test in the orthogroup split and therefore N-masked and unlabelled in train/valid tiles like every test/valid-orthogroup gene); genes whose orthogroup split is more restrictive than the tile are N-masked in the sequence and left out of the label (`leak_masked=n`, protocol A29); since protocol A32 a hard-flagged gene is masked the same way (`hard_masked=n`, tile weight stays 1); A33 adds overlap-component closure (`component_masked`), a seeded flank of [50,150] nt, train-only decoy masks (`decoy_masked`, rate min(0.05, species mask rate / 3)) and drops train/valid tiles whose masked-base fraction exceeds 0.60 (`mask_fraction_dropped`). Before A32 a tile containing a hard-flagged gene (A22) had `train_weight 0`. `window_genes` maps tiles to member genes. RC of a tile reverses every block and re-sorts.
