...
# build_b5.py is loaded by path: importing transgenic.datasets pulls in torch, which the merge does not need.
_B5 = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "src", "transgenic", "datasets", "build_b5.py")
_spec = importlib.util.spec_from_file_location("build_b5", _B5)
...
def stream_hash(con, table, columns, order_by):
    """sha256 over a canonical, ordered, tab-joined dump of `table` — reproducible across rebuilds."""
    h = hashlib.sha256()
    cols = ", ".join(f'"{c}"' for c in columns)
    n = con.sql(f'SELECT count(*) FROM "{table}"').fetchone()[0]
    for off in range(0, n, CHUNK):
        for row in con.sql(f'SELECT {cols} FROM "{table}" ORDER BY {order_by} LIMIT {CHUNK} OFFSET {off}').fetchall():
            h.update(("\t".join("\\N" if v is None else str(v) for v in row) + "\n").encode())
...
def preflight(sources):
    ...
    for path in sources:
        ...
        split_hashes[sp] = stream_hash(con, "gene_split", gs_cols, "species_id, gene_id")
        con.close()
    if len(frozen) != 1:
...
def merge(out, sources, frozen, commits, split_hash, split_table, qc_flags):
    ...
    for i, path in enumerate(sources):
        ...
        # nextval is evaluated per output row; ORDER BY rn keeps the source order, so the merged rn is
        # a stable function of (species order, source rn).
        con.sql(f"INSERT INTO geneList (rn, {quoted}) SELECT nextval('row_id'), {quoted} FROM (SELECT * FROM src.\"geneList\" ORDER BY rn)")
    ...
    content_hash, n_rows = stream_hash(con, "geneList", [c for c in gl_cols if c != "rn"], "species_id, gene_id, is_rc")
    table_hashes = {}
    for t, order in (("gene_key_map", "species_id, gene_id"), ("gene_split", "species_id, gene_id"),
                     ("window_genes", "species_id, window_id, gene_id, is_rc"),
                     ("tile_blocks", "species_id, chromosome, start0"), ("rejected_records", "species_id, gene_id, reason"),
                     ("build_manifest", "species_id")):
        table_hashes[t] = stream_hash(con, t, table_columns(con, t), order)
    con.close()
    manifest = {
