...
MAX_SHADE = 3        # shaded bands per panel; more reads as stripes, not highlight
...
def analyse(locus: str) -> dict:
    ...
    # For every predicted transcript, the AtRTD3 transcripts with the identical CDS
    # chain. This is the confirmation the figure exists to show, so it is carried
    # through to the track labels rather than left implicit in the colours.
    confirms = {}
    for pt, pr in pred.items():
        pc = chain(pr["cds"])
        hits = sorted((t for t, c in art_chains.items() if c == pc), key=suffix)
        if hits:
            confirms[pt] = hits

    return dict(locus=locus, pred=pred, tair=tair, art=art, confirms=confirms,
...
def build_rows(a: dict) -> tuple[list, list]:
    ...
    for t in sorted(a["tair"], key=lambda k: span(a["tair"][k])):
        ...
        if c in seen:
            continue
        seen.add(c)
        # "t1" not ".1": at this locus AtRTD3 ids end in .1/.2 too, and they are not
        # the same transcripts, so the two must not share a label pattern.
        tair_rows.append((f"t{len(tair_rows) + 1}", a["tair"][t], GREY, False))
    rows += tair_rows
    ...
    for t in ([x for x in a["identical"] if x not in placed]
        ...
        art_rows.append((art_label(t), a["art"][t], GREEN, False))
        placed.add(t)
    ...
    for t in pool[:MAX_CONTEXT]:
        art_rows.append((art_label(t), a["art"][t], PALE, False))
        placed.add(t)
    rows += art_rows
...
def headline(a: dict) -> tuple[str, str]:
    ...
    if mode == "reproduced":
        n = len(a["feats"])
        shown = min(n, MAX_SHADE)
        extra = (f"; {n - shown} further intron{'s' if n - shown > 1 else ''} differ"
                 if n > shown else "")
        return ("alternatively spliced intron" + ("s" if n > 1 else ""),
                f"{shown} shaded{extra}" if n > 1 else f"{f0[0]:,}-{f0[1]:,}")
    if mode == "exon":
...
def render(ax, a: dict, *, compact: bool) -> None:
    ...
    width = gx1 - gx0
    pad = width * 0.02
    off = gx0          # draw in bp offset from the locus start: integer ticks, no
                       # decimal kilobases, and the absolute span goes in the axis label
    ...
    # At most MAX_SHADE bands: six overlapping bands read as a striped background
    # rather than as a highlight. The count is stated in the headline instead.
    for s, e in a["feats"][:MAX_SHADE]:
        half = max((e - s) / 2, width * 0.015)
        mid = (s + e) / 2
        ax.axvspan(mid - half - off, mid + half - off,
                   color=SHADE, alpha=0.20, zorder=0, lw=0)

    y = n
    for label, rec, color, hl in rows:
        scaled = {**rec, "cds": [(s - off, e - off) for s, e in rec["cds"]],
                  "utr": [(s - off, e - off) for s, e in rec["utr"]],
                  "exon": [(s - off, e - off) for s, e in rec["exon"]]}
        draw_tx(ax, y, scaled, color, lw=1.2 if hl else 0.7)
        if hl:
            ax.plot(gx1 + pad * 1.2 - off, y,
    ...
    ax.set_xlim(-pad * 1.5, width + pad * 2.5)
    ax.set_ylim(0.35, n + 0.9)
    ax.set_yticks([])
    for sp in ("top", "left", "right"):
        ax.spines[sp].set_visible(False)
    ax.xaxis.set_major_locator(plt.MaxNLocator(5, integer=True, steps=[1, 2, 5, 10]))
    ax.ticklabel_format(axis="x", style="plain", useOffset=False)
    ax.tick_params(axis="x", labelsize=label_fs + 0.6)
    ax.set_xlabel(
        f"bp from locus start   ·   {a['chrom']}:{gx0:,}–{gx1:,} ({a['strand']} strand)"
        f"\n{support_note(a)}",
        fontsize=label_fs + 0.8, labelpad=8, linespacing=1.9)
    # On the multi-panel sheet the long sentence repeats five times and collides with
    # the (A)/(B)/... marker, so the panel keeps only the locus; the kind of event is
    # already spelled out in the amber line directly beneath.
    if compact:
        title = a["locus"]
    elif a["mode"] == "reproduced":
        title = f"{a['locus']}  —  TransGenic isoforms confirmed by AtRTD3 long reads"
    else:
        title = (f"{a['locus']}  —  TransGenic predicts an isoform confirmed by AtRTD3 "
                 f"and absent from TAIR10")
    ax.set_title(title, fontsize=band_fs + (1.0 if compact else 0),
                 fontweight="bold" if compact else "normal",
                 pad=16 if compact else 20)


LEGEND = [(GREY, "TAIR10 reference"),
          (ORANGE, "TransGenic — absent from TAIR10"),
          (BLUE, "TransGenic — reproduces a TAIR10 chain"),
          (DGREEN, "AtRTD3 — confirms the TransGenic chain"),
          (GREEN, "AtRTD3 — carries the marked feature"),
          (PALE, "AtRTD3 — other isoforms")]


def legend_handles(used: set | None = None) -> list:
    """Only the colours a panel actually draws. A key for an unused colour invites the
    reader to hunt for a track that is not there."""
    return [Line2D([0], [0], color=c, lw=4, label=t)
            for c, t in LEGEND if used is None or c in used]


def draw_locus(locus: str) -> dict:
    a = analyse(locus)
    rows, _ = build_rows(a)
    used = {c for _, _, c, _ in rows}
    fig, ax = plt.subplots(figsize=(FIG_W, ROW_H * len(rows) + 1.9))
    render(ax, a, compact=False)
    fig.legend(handles=legend_handles(used), ncol=min(3, len(used)), frameon=False,
               fontsize=6.2, loc="lower center", bbox_to_anchor=(0.5, -0.01))
...
def draw_sheet(loci: list, name: str = "all", ncol: int = 2,
    ...
    for i, (ax, a) in enumerate(zip(flat, analyses)):
        ...
        if panel_labels:
            ax.text(-0.075, 1.055, panel_labels[i], transform=ax.transAxes,
                    fontsize=12, fontweight="bold", va="bottom", ha="right")
    for ax in flat[len(analyses):]:
        ax.axis("off")
    used = {c for a in analyses for _, _, c, _ in build_rows(a)[0]}
    fig.legend(handles=legend_handles(used), ncol=3 if ncol > 1 else 2, frameon=False,
