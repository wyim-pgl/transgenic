...
Protocol matches the benchmark: beam search, num_beams=2, do_sample=False (deterministic),
...
OUTDIR = "/home/framazan/fig3_test_preds_prompted"
DONE = "/home/framazan/fig3_test_preds_prompted.done"
...
# for evaluation, so they belong here.
...
for i in range(0, len(rns), B):
    chunk = rns[i:i+B]
    q = ("SELECT rn, geneModel, start, fin, strand, chromosome, sequence, gff "
         f"FROM geneList WHERE rn IN ({','.join(map(str, chunk))})")
    for rn, gm, st, fin, strand, ch, seq, gff in con.sql(q).fetchall():
        rows[rn] = (gm, st, fin, strand, ch, seq, gff)
...
# Catch-all so an unrecognised prefix is reported and skipped rather than
# aborting the run, as it did in the de novo pass on 2026-07-27.
...
def write_pred(rn, out_row):
    ...
    gm, st, fin, strand, ch, seq, _gff = rows[rn]
...
# ---- prompt construction (mirrors prompt_mode.get_single_decoder_input_ids) --
def first_transcript_ids(label_gsf):
    """Legacy 272-token prompt: features of the first transcript only."""
    ids = [gffTok._convert_token_to_id(t) for t in gffTok._tokenize(label_gsf)]
    labs = ",".join(map(str, ids))
    try:
    ...
    first = ",21,".join(labs.split(",17,")[0].split(",21,")[0:last_element_index + 1])
...
def batch_dii(rns_):
    ...
    for rn in rns_:
        gff = rows[rn][6]
        ids = ([START] + first_transcript_ids(gff)[:2047]) if gff else [START]
        seqs_.append(ids)
    mx = max(len(s) for s in seqs_)
    return torch.tensor([s + [gffTok.pad_token_id or 1] * (mx - len(s)) for s in seqs_], device=device)
...
for bi, batch in enumerate(batches):
    ...
    dii = batch_dii(batch)
    max_new = max(1, 2048 - dii.shape[1])
    try:
        with torch.inference_mode():
            outs = model.generate(inputs=ii, attention_mask=am, num_return_sequences=1,
                                  max_new_tokens=max_new, num_beams=2, do_sample=False,
                                  decoder_input_ids=dii, use_cache=True)
        ...
        for rn in batch:  # fall back to single-row generation
            ...
            with torch.inference_mode():
                o1 = model.generate(inputs=i1, attention_mask=a1, num_return_sequences=1,
                                    max_new_tokens=max(1, 2048 - d1.shape[1]), num_beams=2,
                                    do_sample=False, decoder_input_ids=d1, use_cache=True)
            write_pred(rn, o1)
        n_done += len(batch)
        done_fh.flush()
        continue
