...
$PY "$REPO/revision/scripts/61_univec_trim.py" --fasta "$IN" --hits hits.tsv --out est.univec.fa.gz --report report.tsv --summary summary.json --min-len ${MIN_LEN:-100}
...
{ echo "date=$(date -Is)"; echo "input=$IN"; echo "input_md5=$(cut -d' ' -f1 "$ROOT/est/$SP/est.fa.gz.md5" 2>/dev/null)";
  ...
  echo "trimmer=revision/scripts/61_univec_trim.py min_len=${MIN_LEN:-100} terminal_nt=25"; echo "summary=$(cat summary.json | tr -d '\n ')"; } > PROVENANCE.txt
