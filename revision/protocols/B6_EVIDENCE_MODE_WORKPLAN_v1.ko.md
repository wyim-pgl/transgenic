...
11. 대조군(사전 선언, 같은 코퍼스·같은 초기화·같은 데이터 순서·별도 증강 RNG) [C1-3, C2-Q2]:
   ...
   - C1': 수리 코퍼스의 서열 전용 런(B6와 같은 시드) — 짝 맞춘 비교군;
...
| WP5 | Pilot: 4-GPU, 고정 스텝 예산(≈ 2 epoch), 수리 코퍼스에서 B6 vs C2 vs C1'; 동결된 PILOT 유전자좌 패널(최종 패널과 소)에서 분리된 증거 배분으로 G1/G2 측정; go = pilot 패널에서 두 하한이 임계값 초과, 중단 규칙 미발동 | WP4 | ≈ 3 × 25 GPU-h + 큐 | pilot 보고; go/no-go [C3-4] |
| WP6 | 본 런: B6(11 h 미리 큐잉 체인, A45), C2 전체 길이, C1' 전체 길이; C3/C4 같은 예산 짧은 런; C5 기준선 | WP5 go | 3 × 400 GPU-h + 짧은 런 2개; 잔액 대비 예산 점검 | A28/A45 체인 규칙 |
| WP7 | 평가: 두 모드 × 모든 B1 시대 벤치마크, dose-response, 게이트, A37 arm; 완성 모드 추가 전사체의 B1 검증(C0 표 재사용) | WP6 | 3~4일 | 사전 등록 표 |

임계 경로 ≈ WP0~WP4 ≈ 2주, pilot ≈ 큐 포함 3~4일, 본 런 ≈ 큐 포함 1~2주, 평가 1주 → ≈ 5~6주. GPU 예산(정정 [C3-5]): 잔액 1,433 − 시드 123 사용 ≈ 200 − 시드 123 잔여 ≈ 250 = 가용 ≈ 983; pilot ≈ 75; 본 런 B6 + C2 + C1' ≈ 1,200 → 부족 ≈ 292 GPU-h + 짝 맞춘 짧은 런 2개(C3, C4 ≈ 2 × 30). 저자는 ≈ 350 GPU-h를 보충하거나 C1'을 생략한다(그러면 C1 = 시드 123이 유일한 서열 전용 비교군, 마스크 차이는 수치화).

## 4. 저자가 아직 정해야 할 것
1. 예산: C1' + C2 본 런을 위해 보충할지, C1(시드 123)을 유일한 서열 전용 비교군으로 받아들일지.
