...
| **B5 training corpus (A40)** — `b5_full_a40_v1`, 273,148 rows | `gpu:/home/pgl/scratch1/wyim/transgenic_data/b5/a40_20260905_v1/merged/b5_full_a40_v1.db` (outside the root above) | `dda7e78c880c993435654b57dcd1c498` | 23,534,252,032 |

**B5 corpus frozen and adopted 2026-09-06 (A40 / v1.30).** Recipe `gsf-contract-a40-v1`; the operative corpus and superseded identity are specified in A40. The complete file/source/table hashes, executed source snapshot, invocations and verification are in [`b5_full_a40_v1.freeze.json`](../../data/freeze/b5_full_a40_v1.freeze.json), mirrored beside the new database. Every B5 seed uses this object together with the corrected three-token empty-target tokenizer.
...
## A40. Amendment v1.30 (2026-09-06; author decisions 2026-09-05; replacement build, merge and verification complete, before any B5 seed starts) — the B5 corpus includes no-CDS loci in masking, uses three-token empty targets and shares canonical emitted-block order

**Defects.** The superseded `b5_full_v1` did not implement three parts of its operative contract. First, 6,205 *V. vinifera* no-CDS loci were removed before masking and overlap closure, including 634 valid and 971 test loci whose sequence could bypass A29/A33.3 protection (#56). Second, empty labels stored a three-token count while the decoder tokenizer emitted only `<empty> </s>` (#58). Third, the emitter ordered annotation spans while the validator ordered emitted features and canonical block text; 97 forward windows were rejected for canonical order, comprising 13 equal-span ties and 84 decreasing spans (#55). These are contract defects, not outcome-selected threshold changes.

**Author decision.** On **2026-09-05**, the author chose **#56 B: protect no-CDS loci** ("lncRNA는 마스킹 해") and adopted the recommendations **#58 A: three tokens** and **#55: recover ordering failures in the same replacement build**. On **2026-09-06**, after independent read-only reconciliation against both databases confirmed the three must-not-move quantities, the author approved the freeze and adoption recorded here. The earlier planning hold is superseded. At this amendment, the nine-species build and successful merge are complete and all acceptance checks pass; the 129 completed long-read alignments (approximately 480 GB) remain untouched, and no B5 training run is launched by this record. §§3–9 remain byte-for-byte unchanged.

**Correction (operative): what the corpus now is.** `b5_full_a40_v1` is the nine-species, 273,148-row corpus produced by **`gsf-contract-a40-v1`**, with `tile6144-v3` windows and `isoform-only` RC, the frozen annotation/input files, orthogroup assignments, tiers, caps and masking thresholds retained.

1. **No-CDS loci participate in masking and overlap closure but never carry coding labels.** All eligible loci, including no-CDS loci, enter the species leak+hard rate denominator; leak/hard-eligible loci enter the numerator. Decoy candidates remain otherwise-labelable coding genes. Masking closes over whole overlap-connected components using the existing A33.3 flank and fraction rules. This retains the existing complete-locus scope: edge-partial loci are outside that guarantee, and same-split, non-hard no-CDS loci are not unconditional mask seeds. All 6,205 no-CDS loci participate in the new population; none carries a coding or decoy membership. The full reference GFF for evaluation is unchanged.
2. **Empty decoder targets are exactly three tokens: `<s> <empty> </s>`.** The corrected tokenizer is part of the required run recipe. All 26,057 empty rows were exercised through the dataset decoder path; all 273,148 actual token lengths equal stored counts. Maximum length is 5,920 against the unchanged 8,192 cap; no unknown token, truncation or cap violation was found.
3. **Emitted block order is canonical and shared by emitter and validator.** Ordering is computed from emitted feature spans and canonical block text, covering both equal-span ties and decreasing spans. All labels, including RC labels, pass canonical order and RC involution checks; canonical-order rejections are zero.

**Refreeze.** §1 adopts the new object; the superseded identity remains here for audit and its database, nine sources and freeze JSON remain preserved.

| Identity / quantity | `b5_full_v1` (superseded) | `b5_full_a40_v1` (A40 / v1.30) |
|---|---|---|
| GPU database | `/home/pgl/scratch1/wyim/transgenic_data/b5/merged/b5_full_v1.db` | `/home/pgl/scratch1/wyim/transgenic_data/b5/a40_20260905_v1/merged/b5_full_a40_v1.db` |
| Recipe | `gsf-contract-v1` | `gsf-contract-a40-v1` |
| File MD5 | `ebf4fb511073c546d05e3fa0fc0e4258` | `dda7e78c880c993435654b57dcd1c498` |
| Bytes | **23,491,260,416** | **23,534,252,032** |
| `geneList` rows | **272,224** | **273,148** (+924) |
| `geneList` content SHA256 | `bf6cde9c2e49b5b0ec02f0a0800b8245a5a807822386035bc9fa197a97013617` | `063c9f2f78549dd3d36c692dbc1cf2f6ff1d44816718a51865610f97f5e9e0d0` |
| `gene_key_map` / `gene_split` rows | 334,642 / 334,642 | 334,642 / 334,642 (identical contents) |
| Assigned train / valid / test | 213,702 / 29,153 / 91,787 | 213,702 / 29,153 / 91,787 (identical) |
| Strict holdouts | 3,430 | 3,430 (identical) |
| `rejected_records` | 8,181 | 8,130 (−51) |
| `tile_blocks` | 4,454 | 4,454 (identical contents) |
| `window_genes` | 978,586 | 978,864 (+278) |
| Empty rows | 25,159 | 26,057 |
| Freeze record | `data/freeze/b5_full_v1.freeze.json` (preserved) | `data/freeze/b5_full_a40_v1.freeze.json` |

New database SHA256: `6840c93fd0b999e09b8dc2897adca1f4dbae36f2ef0f8fcc10ee66371295e222`. Assignment content SHA256 remains `61766859a1d712b49f3fdbeb457067d00e604d53a0298af614c28bff2ea1deee`.

**Recovery accounting.** **19 forward + 19 RC ordering recoveries survived into the final corpus, not 97.** Those are the 8 *A. thaliana*, 10 *P. patens* and 1 *P. trichocarpa* historical rejected coordinates and their RC rows. All 78 historical *V. vinifera* coordinates disappear after the no-CDS population correction changes empty-sampling RNG consumption and later tile offsets. In the explicitly ordered counterfactual **historical → #55 with old population → #55 + #56**, ordering contributes 97 forward +63 RC =160 and the population change contributes net +764, totalling +924. The counterfactual is not an adopted corpus and its 97 forward windows are not a final-retention count. Equivalently, direct final recovery is +38 and the *V. vinifera* population/order interaction is +886. Unexplained additions, removals and shared-row semantic differences are all zero.

**Verification and provenance.** The [A40 verification note](../../notes/a40_verification_20260906.md) and its linked audit files record nine exact current-source replays, nine fresh source validations, zero merged-validator violations, exhaustive token/RC/membership checks, independent no-CDS N-coverage and component closure, and exact FASTA-edge-aware margin diagnostics. The three must-not-move quantities held: 334,642 nominal assignments, their train/valid/test partition, and 3,430 strict holdouts. The author-facing reconciliation was independently checked read-only by the user before freeze approval. The freeze-time pass recomputed the merged file hashes, all seven table hashes and all nine source DB hashes against the merge receipt.

The source identity is **HEAD `aa2623f215d4556478395dee6cf63da5a78c65f3` plus the retained uncommitted patch and SHA256 inventories**, not HEAD alone. The species build used the clean archived HEAD snapshot (`provenance/working-tree.patch` is empty); the successful merger ran from `audit/verification-source` with the completion-evidence gate patch in `scripts/merge_b5_databases.py` and its tests. The freeze embeds that patch, its SHA256, both source inventories and auxiliary source hashes, and the actual build/merge argv, cwd, timestamps and exit status. It also retains the failed initial missing-`.DONE` invocation as failed history; the accepted merge used `--build-status`, fresh validation and verified completion evidence. No fabricated marker or in-place database patch was used. The original merge receipt and pre-adoption audit status are preserved as historical records, with this freeze recording adoption separately.

**B5 run disposition.** The corpus/tokenizer blockers #55, #56 and #58 are resolved by this rebuild and freeze. A B5 run may now start using **the new §1 database, the corrected three-token tokenizer and a new labelled run** with the frozen data/source identity recorded for every seed. **A28 forbids patching this changed recipe inside an existing chain**; a superseded-data/tokenizer chain cannot silently become an A40 run. The ACCESS resource and ordinary run prerequisites still apply. This amendment does not settle #61's separate scoring-universe question or redefine any evaluation denominator.

## A41. Amendment v1.31 (2026-09-08; alignment in progress, before any protein-evidence result table and before #45 or #44 is closed) - the OrthoDB protein alignment runs on pronghorn and executes in six query chunks; A19.2 parameters are unchanged and the partition was measured to be result-neutral

**Run location (the section 12 record A19.2 required).** A19 states that the OrthoDB resource is "downloaded on the ACCESS Delta allocation, where alignment runs", and #45 says "miniprot per training genome on Delta". The alignment runs on **pronghorn**. The author decided this on 2026-09-07 after the sizing pilot: the input is the section 1 frozen object identified by md5, so the location cannot change the input; miniprot is CPU work that would otherwise spend GPU allocation; and A19.4 item 3 places this "parallel with B5 training" rather than on the same machine. The deviation was recorded in the driver, in every `PROVENANCE.txt` and in `PROVENANCE.tsv` from the first run. This amendment is the protocol record those files pointed to.

**Why the query set is now partitioned.** Array `6150892` ran the whole 12,115,085-sequence resource per species at `--mem=64G` and lost three species to `OUT_OF_MEMORY`: *B. distachyon* (53.9 GiB), *G. max* (55.2 GiB) and *O. sativa* (63.0 GiB, sampled peaks; the sampler undercounts the true peak). miniprot memory **does not track genome size** - the first two to die (275 MB, 382 MB) are smaller genomes than one that was still running (1,024 MB) - and it does not rise smoothly: *G. max* held 29.9 GiB for hours, then stepped to 55.2 GiB across 0.35 M queries and died there. No ceiling inferred from an observed plateau is therefore sound. `lh3/miniprot#68` is the same signature upstream, on a host with 925 GB available, and is closed with no fix and no workaround offered. The five surviving tasks were cancelled by author decision on 2026-09-08 and all nine species restarted chunked.

**The sizing error, recorded.** The two-point pilot behind `--mem=64G` varied genome size 1x to 8.45x while **holding the query count fixed at 100,000**. It could not observe the axis that caused the failure, and the driver then stated as fact - "memory is dominated by the index and the streamed batch, not by the query count" - a property that had never been measured. This is the same shape as the long-read OOM that extrapolated EST performance (`quarantine.md` section 2f). The rule this establishes: an axis that was held fixed has not been measured flat, it has not been measured.

**Partition (operative).** The frozen section 1 protein resource (`odb12_Viridiplantae.filtered.fa.gz`, md5 `453cb32b02e0799950d7d5f4de5f62ac`, 12,115,085 sequences) is split **in input order** into six chunks of 2,019,181, the sixth 2,019,180, recorded with a per-chunk md5 in `evidence/protein/chunks_v1/MANIFEST.tsv`. The split is verified twice: the chunk record counts must sum to 12,115,085, and the **uncompressed chunks concatenated in order must md5-match the uncompressed source**. The first check proves nothing was lost; only the second proves nothing was reordered or altered.

**Result neutrality was measured, not assumed.** Job `6152995` ran three arms against the *A. thaliana* pilot index: **A**, the existing 1 x 100,000; **B**, a fresh re-run of the same 100,000, as a control for `-t 32` nondeterminism; and **C**, 2 x 50,000 concatenated. A, B and C are identical at the GFF level - same record count (643,907), same order, identical md5 once the run-local `MP######` ids are dropped - and identical through `protein_accept.py`: coverage intervals, introns, `mapping_ambiguous`, `alignments.tsv` modulo `alignment_id` and `summary.json` all match, at 100,851 alignments, 78,847 accepted, 76,183 proteins. A being identical to B also establishes that miniprot at `-t 32` is byte-reproducible, which was not previously known. Evidence: `evidence/protein_alignments/pilot/split_equiv/REPORT.txt`.

**Merge rule (operative).** `MP######` ids restart at 1 in every chunk - 46,833 collisions in the two-way pilot. `protein_merge.sbatch` renumbers with a running offset so merged ids are contiguous and unique, and it rewrites **only** `ID=` and `Parent=` in column 9. A blanket substitution on `MP[0-9]+` is forbidden: `##PAF` lines carry `MP` inside query **names** (578 such lines in the pilot chunk) and would be rewritten as data. The merge refuses unless every chunk is complete and built from the same reference md5 and the same parameters, and unless merged mRNA count, distinct id count and maximum id all agree. **A chunk is not a result**: only the merged per-species GFF carries `DONE` and `PROVENANCE.txt`.

**Unchanged.** The A19.2 frozen parameters `--gff --outs=0.97 -N 20` and the acceptance thresholds (identity >= 30 %, coverage >= 70 %, no frameshift, canonical introns <= 200 kb) are unchanged. Sections 3-9 are unchanged. No B5 corpus, training run or evaluation denominator is affected by this amendment.

## A42. Amendment v1.32 (2026-09-08; before any B5 benchmark or seed starts on DeltaAI) - `configs/b5_400m_win_v3.json` is the B5 recipe file; the benchmark measures that file, one window tier per run

**Erratum to A25.** A25 names `configs/b5_400m_ctx_v2.json` as the B5 recipe. A26 and A27 then changed what a B5 row is - whole-window GSF v3 labels (`tile6144-v3`), vocabulary 290, decoder positions and label cap 8,192, a 96-gene cap - and the A40 corpus was built with `tile6144-v3`. Those properties exist only in `configs/b5_400m_win_v3.json`; `ctx_v2` still carries `tier6144-v2`, the single-target-gene labelling the A40 rows were not built with. The launch script trained win_v3 and the preflight already called it the canonical recipe, but the frozen text still said ctx_v2 and the benchmark script still measured ctx_v2. **Operative: `configs/b5_400m_win_v3.json` is the B5 recipe file for every seed and every benchmark. `ctx_v2` is retained for reference only.** A25's other provisions - the three window tiers 30,720 / 61,440 / 129,024 nt, ACCESS as the compute, and "#18 benchmarks the three tiers separately before the seeds start" - are unchanged.

**Benchmark rule (operative).** `bench_b5.slurm` measures win_v3 and pins one tier per run with `--benchmark-tier` (the trainer selects rows whose tile length equals the tier exactly; an unpinned run samples a tier mixture under `tier_up_prob` 0.3 and is not a per-tier number). Six runs before the first seed: three tiers x {1 GPU, 4 GPUs}. Measuring a cheaper or differently labelled configuration and extrapolating to the training run is the same error A41 records for the protein alignment.

**DeltaAI launch corrections recorded here because they change what a seed costs or whether it survives.** (1) Under `set -e`, `wait` returns 138 when the pre-limit `USR1` arrives and errexit ended `train_b5.slurm` before the resubmit; every seed would have stopped at 48 h with no next link. The wait is now re-entered while the trainer lives, so A28 chaining functions. (2) `ghx4` bills with `MAX_TRES` over CPU 13.89, memory 9.017 per GB and GPU 1000: `--mem=200g` charged a 1-GPU job as 1.80 GPUs. Memory requests are removed; `DefMemPerGPU` (110.9 GB per GPU) applies and scales with the GPU count. (3) The GPU count is read from `SLURM_GPUS_ON_NODE`, a bare integer set for every request form; `SLURM_GPUS_PER_NODE` echoes the submitter's string and is unset for `--gres`. (4) `#SBATCH` directives do not expand shell variables; the GPU count is fixed at 1 in the directive and overridden on the command line, and the chain resubmit repeats it. (5) DeltaAI has no `/scratch`, no `$SCRATCH` and no apptainer module; the scripts default to `/work/nvme/bilv/$USER` and `/usr/bin/apptainer`. (6) `cpu_tools.slurm` cannot run on DeltaAI (no CPU partition; GPU-less jobs are refused) and is marked Delta-only; the protein alignment runs on pronghorn under A41. Sections 3-9, A19.2 and the B5 corpus are unchanged.

## A43. Amendment v1.33 (2026-09-09; after the first protein-evidence result table, before any protein-derived label or weight is used) - the locus-specific protein support tables exclude `mapping_ambiguous` proteins before aggregation (support_version 2); the version-1 tables leaked them

**Defect.** A19.2 says equal-best multi-locus placements are `mapping_ambiguous` and count at family level only. `protein_accept.py` (version 1, array 6154025) wrote each accepted alignment's CDS blocks and introns during its single streaming pass and identified ambiguous proteins only afterwards, so their accepted alignments entered `<species>.tsv` (per-base distinct-organism coverage) and `<species>.introns.tsv` (>= 2-organism intron boundaries). Those tables carry no protein identity, so the contribution could not be subtracted after the fact. Found by an independent code review (Codex, 2026-09-09) of the #45 scripts; the acceptance fractions in `protein_acceptance_v1.tsv` were never affected.

**Change (operative).** `protein_accept.py` support_version 2 (commit 5bc4df8): block and intron rows carry the protein id through the streaming pass; once every alignment of a protein has been seen, rows of `mapping_ambiguous` proteins are removed before the sort, the coverage sweep and the intron table. The summary records `support_version`, `support_excludes_mapping_ambiguous`, `ambiguous_alignments_excluded`, `ambiguous_blocks_excluded`, `ambiguous_introns_excluded`. Re-run as array 6156132 over the same nine merged GFFs (gff_md5 unchanged); results in `revision/results/evidence/protein_acceptance_v2.tsv`, which supersedes v1. Version-1 outputs are preserved at `evidence/protein_alignments/<species>/accept_v1_ambiguous_leak/` and must not feed labels or weights.

**Effect.** Per species 1.4-4.1 % of proteins are `mapping_ambiguous`; excluding their accepted alignments (2.4-6.0 % of accepted alignments) lowers coverage intervals by 2-7 % and >= 2-organism introns by 1.3-4.1 % (e.g. Athaliana 345,836 -> 341,479; Gmax 619,329 -> 603,056). A19.2 parameters, thresholds and the alignments themselves are unchanged. The A18.4 organism count `n` and the A19.3 intron classes are to be computed from the version-2 tables only.

## A44. Amendment v1.34 (2026-09-10; seed 123 in its second epoch on NGC 25.06, before seeds 456 and 789 start) - the confirmation seeds run on NGC 26.08 with torch.compile and 4-GPU DDP from a second frozen checkout; the replication is a mixed-runtime replication, and the job chain, checkpoint publication and epoch-boundary accumulation are corrected

**Author decision (2026-09-10).** Seeds 456 and 789 run on NCSA DeltaAI in the NGC `pytorch:26.08-py3` image (`transgenic-ngc-26.08.sif`, sha256 `1a5dee27b8575efd...`; torch 2.14.0a0+4fdf77b940.nv26.08, transformers 4.57.6, accelerate 1.15.0) with `torch.compile` on the whole model (the `suppress_errors` fallback policy of A42 unchanged), on four GH200 GPUs under DDP (`find_unused_parameters=True`), per-rank gradient accumulation 24 so the effective batch stays 96, non-reentrant activation checkpointing, from the frozen checkout `repo_seeds` (commit recorded in each run's `runtime_identity.txt`). Seed 123 continues unchanged: NGC 25.06 (`transgenic-ngc.sif`, sha256 `3b242b339f59a8f8...`; torch 2.8.0a0), eager (compile falls back on that image), one GPU with accumulation 96, reentrant checkpointing, checkout `08f3e2b` with the chain script of commit `ba24b19`. Recipe file, database (§1, A40), tokenizer, splits, seed roles, learning-rate schedule, effective batch and the best-validation stop rule (patience 3) are identical for all three seeds.

**Why.** On 25.06 `torch.compile` fails in the AOTAutograd partitioner for this model (activation checkpointing + dropout; pytorch#135459 family; also fails on 26.04/torch 2.12.0a0 and with per-layer compile) and the trainer runs eager. On 26.08 it compiles: 1 GPU 21.35 / 40.54 / 86.64 s per optimizer step at the 30,720 / 61,440 / 129,024 nt tiers (1.12-1.17x over 25.06 eager), 4 GPUs 5.89 / 11.04 / 23.23 s (3.62-3.73x over 1 GPU, 91-93 % scaling), i.e. 6.15 h wall and 24.6 GPU-h per epoch, 197-246 GPU-h per seed. Measured with `deploy/deltaai/bench_b5.slurm`, 50 optimizer steps with the first 20 discarded, 2026-09-09.

**What the decision costs (recorded, not claimed away).** The three seeds no longer differ by seed alone: image, torch version and compiler kernels, gradient-reduction order across four ranks, and the RNG/dropout streams of four processes differ between seed 123 and seeds 456/789. No bitwise or measured result-neutrality of these runtime changes is claimed. The replication statement in the manuscript must say that seed 123 ran on one GPU without compilation and seeds 456/789 on four GPUs with compilation, and compare the seeds on the validation-loss trajectory, the stop epoch and the downstream evaluation (A19.4 and later), not on step-level numerics. A same-runtime seed pair is the way to separate seed variance from runtime variance if that question is ever asked.

**Corrections that apply to the confirmation seeds (all measured, 2026-09-09/10).**
1. The A28 job chain had never completed a 48 h boundary: the batch script masked every trainer exit code as 0, its pre-limit signal killed the container (rc 138) without reaching the trainer, and a rank under dynamo never runs its SIGUSR1 handler. The chain script now forwards SIGTERM to the rank processes only, keeps the real exit code, retries a first-launch `unspecified launch failure` inside the allocation, forces a preemption from the last `latest_state` when the trainer ignores the signal, and refuses a later link whose runtime identity (image sha256, checkout, GPUs, CPUs, torch, checkpointing and compile policy) differs. Rehearsed with 15-minute limits on the frozen 25.06 trainer (jobs 3120089, 3120124, 3120196) and on the 26.08 four-GPU configuration (this amendment's own rehearsal job id in the run log). Seed 123's frozen checkout carries the script of `ba24b19`.
2. `latest_state` is published by renames only (old -> `.prev`, new -> `latest_state`); a kill between the old delete and the rename used to leave no resumable checkpoint.
3. The gradient-accumulation window carries across epoch boundaries in both runtimes (198,828 training rows are not a multiple of 96: 12 micro-batches pend at every epoch end on one GPU, 3 per rank on four). Seeds 456/789 persist that window in the epoch checkpoint (`pending_grads/rank<i>.safetensors`, `pending_micro` in `meta.json`) and restore it on an epoch-checkpoint resume, so their update boundaries are those of uninterrupted training. Seed 123's frozen trainer does not persist it: a resume from one of its epoch checkpoints would shift every later update by 12 micro-batches and drop those 12 samples once. Its chain resumes from `latest_state` (saved at optimizer boundaries), so this occurs only if an epoch checkpoint is ever the newest resumable state; if it does occur, it is recorded in the run log as a deviation.
4. Seed 123's `train_epoch_loss` lines for epochs >= 2 divide by the cumulative micro-batch count and are under-reported by the epoch index; the validation loss (the stop criterion) is unaffected. Seeds 456/789 report the per-epoch average. Seed 123's train-loss series is corrected in the results table, not in the log.

**Epoch ceiling (author, 2026-09-10).** The 22 in A18.6 and in `configs/b5_400m_win_v3.json` (`max_epochs`) is the length of the original training run behind the released checkpoints, which ran 22 epochs with no early stopping. Under B5 the epoch count is produced by the stop rule itself -- minimum mean validation loss with patience 3 -- and 22 is only the ceiling that run may not exceed; `revision/scripts/60_epoch_budget.py` applies the same rule (stop-or-plateau epoch + 2, capped at 22) when it reports a budget.

**Runtime identity.** Every seed's first link writes `runtime_identity.txt` next to `run_config.json`; the identity of each seed is copied into the results table with the run log. Rows of the results table carry `seed`, `image`, `torch`, `gpus`, `checkout`.

## A45. Amendment v1.35 (2026-09-11; seed 123's first 48 h link has ended and its 1-GPU link 2 is queued unstarted; seeds 456/789 are queued unstarted on 4 GPUs) -- the primary seed restarts on the A44 runtime, so the three seeds share one runtime identity, and every seed's chain is pre-queued

**Author decisions (2026-09-11).** (a) "우리 그렇게 시간이 없어": seed 123 does not continue on one GPU. (b) The allocation was topped up (`bilv-dtai-gh` balance 1,433 of 1,499 GPU-hours on 2026-09-11 09:5x PDT), so three 4-GPU seeds (197–246 GPU-hours each) fit. (c) The chains are queued up front so a link never waits for its predecessor to submit it; the corrections Codex found in that plan (2026-09-11) are applied here.

**What A44 left in place, and why it changes.** A44 kept seed 123 on NGC 25.06, eager, one GPU (frozen checkout `08f3e2b`, chain script `ba24b19`) and called the replication mixed-runtime. Measured on that runtime an epoch takes 25.5 h; seed 123's first link ended at its 48 h limit on 2026-09-10 21:45 PDT during epoch 2 (81 %), and because Slurm snapshots the batch script at submission, that link ran the original 2026-09-08 script text, whose `wait` returned on the trap: the trainer was killed without a boundary save and the last `latest_state` (19:51 PDT, global_step 3,600) would have cost ≈ 1.9 h on resume. At 25.5 h per epoch and one queue wait per 48 h link, the remaining 6–8 epochs are ≈ 12 days; on the A44 runtime (6.15 h per epoch, four GPUs, 24.6 GPU-hours) they are ≈ 5.5 days including queue waits. Line 273 governs which seed is reported, not the hardware it runs on; A28 forbids changing a running chain, not ending one and starting another.

**Rule.**
1. Seed 123 restarts from scratch on the A44 runtime: NGC 26.08 (`transgenic-ngc-26.08.sif`, torch 2.14.0a0), `torch.compile` on, four-GPU DDP with per-rank accumulation 24, non-reentrant checkpointing, checkout `repo_seeds` at the commit that carries this amendment. Its queued 1-GPU link 2 (job 3128376) is cancelled before it starts. The three seeds therefore share one `runtime_identity.txt` up to the seed, and A44's "mixed-runtime replication" wording no longer applies: the replication is same-runtime. The reported checkpoint and the main-text numbers still come from seed 123 (line 273).
2. The 1-GPU partial run of seed 123 (epoch 1 complete, validation loss 0.8617, epoch 2 to 81 %) is moved aside by the operator to `runs/seed123_1gpu_partial` before the new chain is submitted (`submit_chain.sh` refuses an existing `runs/seed123`), is preserved unchanged there, and is **not** a reported seed. Its epoch-1 validation loss may be quoted next to the new run's epoch-1 value as a descriptive note in the run log; it is not a runtime-equivalence test and no result-neutrality between runtimes is claimed (A44).
3. Every seed is launched by `deploy/deltaai/submit_chain.sh SEED 3`: three 48 h links (144 h ≥ the 22-epoch ceiling at 6.15 h per epoch = 135 h) queued at once, link k+1 with `--dependency=afterany:<link k>`, every link with `--comment=seed<SEED>`, `CHAIN_N=k`, `CHAIN_MAX=3` and `CHAIN_PRESUBMITTED=1`. Nine jobs for three seeds. The submitter refuses a seed that already has queued or running links, an existing run directory (unless `RESUME_EXISTING=1`), or a `TRAINING_DONE`/`TRAINING_FAILED` marker.
4. `train_b5.slurm` in pre-queued mode (`CHAIN_PRESUBMITTED=1`) never calls `sbatch`. Its exit paths, which is what `afterany` sees: a link that reaches its limit and saves cleanly exits 0; a forced preemption (the watchdog killed a trainer that ignored SIGTERM) consumes its marker and exits 0; a second `unspecified launch failure` on the node writes the node into `EXCLUDE_NODES`, updates `ExcNodeList` of every queued successor with `scontrol update`, and exits 1 -- the successor repeats the same work from the last published checkpoint on another node; a trainer failure writes `TRAINING_FAILED`, cancels every queued successor (`scancel`) and exits 1; a finished run (`TRAINING_DONE`) cancels the queued successors and exits 0. `afterany`, not `afterok`, because the forced-preemption and launch-failure exits must be continued, and a genuine failure is stopped by the marker, not by the dependency.
5. Before anything touches the GPUs, every link checks the run directory: `TRAINING_DONE` → "nothing to do", exit 0; `TRAINING_FAILED` → REFUSED, exit 1; a `LINK_ACTIVE.<jobid>` marker of a job that Slurm still reports in the queue, or whose state cannot be confirmed because `squeue` failed → REFUSED, exit 3 (two links of one seed never share a run directory; the check fails closed); a marker whose job Slurm no longer knows is removed. The link records its own marker and removes it on exit. The submission's `--exclude` list seeds `EXCLUDE_NODES` so a later exclusion extends it.
6. The self-resubmitting chain of A28/A44 (no `CHAIN_PRESUBMITTED`) is unchanged and remains available; `tests/test_train_b5_chain.py` covers both modes (34 tests) and the guard.
7. No scheduling gain beyond immediate eligibility is claimed: DeltaAI's priority is fairshare-dominated (weights fairshare 10,000, partition 11,000, age 1,000) and a job waiting on a dependency does not accrue age (`ACCRUE_ALWAYS` is not set). The time saving of this amendment comes from the runtime (item 1), which cuts a seed from ≈ 12 links-worth of waiting on one GPU to two or three links on four.

**Certification.** The 4-GPU 26.08 chain, epoch-checkpoint resume and integration tests of A44 (jobs 3124229, 3124347, 3124348, checkout `c646e76`) cover the trainer and the signal path, which this amendment does not touch. The pre-queued mode is rehearsed before the seeds are submitted with a three-link 15-minute chain (`--time=00:15:00 --signal=B:USR1@600`, seed 999): link 1 ends by SIGTERM and exits 0, link 2 resumes `latest_state`, a `TRAINING_DONE` written during link 2 makes it cancel link 3, and a fourth link submitted afterwards exits at the guard. Go/no-go: every one of those four outcomes must be observed in the job outputs (`exit 0` in sacct for links 1 and 2, `Resumed from` in link 2's stderr, link 3 CANCELLED by link 2, the fourth link's "nothing to do"); any deviation stops the launch until its cause is understood and the rehearsal is repeated. The rehearsal job ids are recorded in the run log with the seeds' job ids. Sections 3–9 are unchanged.

| Date | Analysis state | Change | Reason |
|---|---|---|---|
| 2026-09-06 (author decisions 2026-09-05) | Nine-species rebuild, merge and full verification complete; long-read alignments complete and preserved; before any B5 seed starts | v1.30: A40 adopts `b5_full_a40_v1` in §1 and freezes the no-CDS masking population, three-token empty targets and shared canonical emitted-block order | #55/#56/#58 contract defects; author-approved replacement after independent reconciliation; all three must-not-move quantities held; final direct ordering recovery 19 forward +19 RC |
| 2026-09-08 | Protein alignment restarted chunked and in progress; no protein-evidence result table produced yet; B5 corpus and long-read alignments untouched | v1.31: A41 records that the OrthoDB alignment runs on pronghorn rather than Delta, and partitions the query set into six chunks with a measured result-neutrality check and an id-renumbering merge rule | Array 6150892 lost three species to OUT_OF_MEMORY at 64 GB; miniprot memory is unbounded in the query set and lh3/miniprot#68 is closed with no fix; the original pilot held the query count fixed and could not see that axis |
| 2026-09-08 | Chunked protein alignment in progress; DeltaAI allocation `bilv-dtai-gh` active; bundle transferred; no benchmark or seed started | v1.32: A42 makes `configs/b5_400m_win_v3.json` the B5 recipe file (erratum to A25), requires the benchmark to measure it one tier per run, and records six DeltaAI launch corrections (errexit killed the A28 chain at USR1; `--mem` billed 1.8 GPUs under MAX_TRES; GPU count from SLURM_GPUS_ON_NODE; no `$SCRATCH`/apptainer module; cpu_tools Delta-only) | Independent review of the launch bundle before the first GPU-hour was spent; the frozen text named ctx_v2 while the scripts trained win_v3 |
| 2026-09-09 | First protein-evidence result table produced (v1); no protein-derived label or A18.4 weight computed yet; B5 seed 123 training on DeltaAI, untouched | v1.33: A43 removes `mapping_ambiguous` proteins from the locus-specific protein support tables before aggregation (support_version 2) and supersedes `protein_acceptance_v1.tsv` with v2; alignments and acceptance unchanged | Independent code review of the #45 scripts found that version 1 wrote support rows before ambiguity was known (A19.2: family level only) |
| 2026-09-10 | Seed 123 in epoch 2 on NGC 25.06 (1 GPU, eager); seeds 456/789 not started; protein evidence v2 complete; DeltaAI benches and DDP integration tests done | v1.34: A44 -- confirmation seeds on NGC 26.08 with torch.compile and 4-GPU DDP from a second frozen checkout (mixed-runtime replication, stated as such); chain, checkpoint-publication and epoch-boundary accumulation corrections; runtime identity recorded per run | Author decision 2026-09-10 (image 26.08, four GPUs); the 48 h chain had never been exercised and a rehearsal showed it broken; Codex readiness review 2026-09-10 |
| 2026-09-11 | Seed 123 link 1 ended at 48 h during epoch 2 (81 %), its 1-GPU link 2 queued unstarted; seeds 456/789 queued on 4 GPUs, unstarted; allocation topped up | v1.35: A45 -- seed 123 restarts on the A44 runtime (26.08, compile, 4-GPU DDP, `repo_seeds`), the 1-GPU partial run is preserved and not reported, every seed is launched as a pre-queued three-link chain (`submit_chain.sh`, `afterany`, `--comment`, `CHAIN_PRESUBMITTED=1`) with a start-of-link guard, a per-seed concurrency lock and exclusion propagation | author: time ("우리 그렇게 시간이 없어"); 25.5 h vs 6.15 h per epoch; Codex plan review 2026-09-11 (approve-with-changes, all changes applied) |
