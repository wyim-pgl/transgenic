# ruleout.md — 기각·제외 원장 (rule-out ledger)

> 📌 **정본**: 이 파일은 **"쓰지 않기로 한 것"의 정본**이다. "지금 무엇을 쓰는가"의 정본은
...
전수 목록은 `grep -n 'SUPERSEDED\|CAUTION (\|PARTIAL (' README.md ruleout.md`.
...
- **#55 · #56** 둘 다 빌더 수정 = 고정 레시피 개정 + 9종 재빌드(약 1.5시간). 지금 할지 다음 빌드로
  미룰지 저자 결정.
- **#58** `<empty>` 비대칭이 의도인가. 의도라면 `count_tokens_v3`·스펙·테스트를 2로 고치는 것이고,
  아니라면 토크나이저 한 줄이며 **DB 재빌드는 불필요**하다.
- **#60** subreads 51런 43 GB를 제외할지, consensus 단계를 범위에 넣을지. 제외하면 *S. bicolor* 장독 증거 0.
- **#61** 논문이 "test set"을 어느 정의로 기술할지(명목 orthogroup split vs 실제 라벨된 16.3 %).
- **ACCESS 자원명·계정·파티션** 미확인 — #51과 #19를 막고 있다.
- **원고 Methods**: 인코더는 부분 사전학습이므로 **"partially pretrained"**로 고쳐야 한다(A34, 미반영).

---

## 8. 원고·제출 측 기각 — ⚠️ 이력에서 인계, 이번 세션 재검증 없음
...
> §1–§7은 이번 세션에서 코드·데이터로 직접 확인한 것이고, 이 절만 성격이 다르다.
