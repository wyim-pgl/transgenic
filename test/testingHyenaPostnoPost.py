...
# "checkpoints/Hyena_SegmentFocalDice_Wide_E5.safetensors"
...
with safe_open(segment_checkpoint, framework="pt", device="cpu") as f:
	for k in f.keys():
		if "segment" in k:
			segmentation_tensors["transgenic.encoder." + k.replace("_orig_mod.", "")] = f.get_tensor(k)
...
def greedySearch(batch):
	ii, am, lab = batch[0].to(device), batch[1].to(device), batch[2]
	with torch.no_grad():
		#outputs = model(input_ids=ii, attention_mask=am, global_attention_mask=gam, labels=lab, return_dict=True)
		outputs = model.generate(
					inputs=ii, 
...
for step, batch in enumerate(tqdm(test_data)):
	...
	try:
		...
		#pred, true, sequence = beamSearch(batch)
	except:
