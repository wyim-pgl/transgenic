...
class GFFTokenizer1(PreTrainedTokenizer):
	model_input_names = ["input_ids", "attention_mask"]

	def __init__(self, vocab=None, **kwargs):
		if vocab is None:
			self.vocab = {
				"<s>": 0, "<pad>": 1,"</s>":2, "<unk>":3, 'A': 4, 'B': 5, 'C': 6, ">":7, ".": 8, 
				"+": 9, "-": 10, ";":11
			}
			for i in range(1, 151):
				self.vocab[f"CDS{i}"] = i + 11
			for i in range(1, 51):
				self.vocab[f"five_prime_UTR{i}"] = i + 161
				self.vocab[f"three_prime_UTR{i}"] = i + 211
			for i in range(0, 50000):
				self.vocab[str(i)] = i + 262
		else:
			self.vocab = vocab
